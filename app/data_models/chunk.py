...
class ChunkCreate(ChunkBase):
    metadata: ChunkMetadata | None = Field(default=None, description="Chunk metadata")


class ChunkUpdate(BaseModel):
    text: str | None = Field(default=None, description="New text content")
    metadata: ChunkMetadata | None = Field(default=None, description="Updated chunk metadata")
...
class Chunk(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique identifier for the chunk"
    )
    document_id: UUID = Field(..., description="ID of the document this chunk belongs to")
    text: str = Field(..., description="Text content of the chunk")
    embedding: list[float] | None = Field(
